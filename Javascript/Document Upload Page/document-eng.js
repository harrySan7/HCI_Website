document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateFile(file, type) {
        ...
        if (file.size > maxSize) {
            showNotification(`File ${file.name} is too large. Maximum is ${type === 'bank' ? '10MB' : '5MB'}.`, 'error');
            return false;
        }

        if (!allowedTypes.includes(file.type)) {
            showNotification(`File format ${file.name} is not supported.`, 'error');
            return false;
        }
    ...
    function createFilePreviewModal(file) {
        ...
        if (file.type.startsWith('image/')) {
            ...
            pdfInfo.innerHTML = `
                ...
                <h4 style="color: var(--text-color); margin-bottom: 15px;">PDF File</h4>
                <p style="color: var(--gray-500); margin-bottom: 20px;">
                    To view the full PDF content, please download the file.
                </p>
            ...
            unsupportedInfo.innerHTML = `
                ...
                <h4 style="color: var(--text-color); margin-bottom: 15px;">Preview not available</h4>
                <p style="color: var(--gray-500);">This file type cannot be previewed, but the file has been uploaded successfully.</p>
            `;
    ...
    function showSuccessMessage() {
        ...
        formContainer.innerHTML = `
            <div style="text-align: center; padding: 3rem 2rem; background-color: rgba(255, 255, 255, 0.95); border-radius: 20px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.1);">
                ...
                <h2 style="color: var(--success); font-size: 2.5rem; margin: 1.5rem 0 1rem; font-weight: 700;">Documents Successfully Sent!</h2>
                <p style="color: var(--text-color); font-size: 1.3rem; margin-bottom: 2rem; line-height: 1.6;">
                    Thank you! Your documents have been uploaded successfully. Our verification team will review your documents within <strong>2-3 business days</strong>.
                </p>
                
                <div style="background: var(--gray-100); padding: 2rem; border-radius: 16px; margin: 2rem 0; border: 2px solid var(--primary-color);">
                    <h3 style="color: var(--primary-color); margin-bottom: 1.5rem; font-size: 1.4rem; font-weight: 600;">Document Status:</h3>
                    <div style="text-align: left; display: grid; gap: 1rem;">
                        <div style="display: flex; align-items: center; gap: 12px;">
                            ...
                            <span style="font-size: 1.1rem; color: var(--text-color);">KTP - Successfully uploaded</span>
                        </div>
                        <div style="display: flex; align-items: center; gap: 12px;">
                            <div style="width: 32px; height: 32px; background: var(--success); border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                                <i class="fas fa-check" style="color: white; font-size: 0.9rem;"></i>
                            </div>
                            <span style="font-size: 1.1rem; color: var(--text-color);">SIUP/NIB - Successfully uploaded</span>
                        </div>
                        <div style="display: flex; align-items: center; gap: 12px;">
                            <div style="width: 32px; height: 32px; background: var(--success); border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                                <i class="fas fa-check" style="color: white; font-size: 0.9rem;"></i>
                            </div>
                            <span style="font-size: 1.1rem; color: var(--text-color);">Bank Statement - Successfully uploaded</span>
                        </div>
                        <div style="display: flex; align-items: center; gap: 12px;">
                            <div style="width: 32px; height: 32px; background: var(--success); border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                                <i class="fas fa-check" style="color: white; font-size: 0.9rem;"></i>
                            </div>
                            <span style="font-size: 1.1rem; color: var(--text-color);">Store Photo - Successfully uploaded</span>
                        </div>
                    </div>
                </div>
                ...
                <div style="background: rgba(144, 31, 31, 0.05); padding: 1.5rem; border-radius: 12px; margin: 1.5rem 0; border-left: 4px solid var(--primary-color);">
                    <p style="color: var(--text-color); font-size: 1rem; margin: 0; display: flex; align-items: center; gap: 8px;">
                        ...
                        <strong>Note:</strong> You will receive an email and WhatsApp notification after the verification process is complete.
                    </p>
                </div>
                
                <div style="display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap; margin-top: 2rem;">
                    <button onclick="location.href='/HTML/progress-track/progress-eng.html'" style="background: var(--primary-color); color: white; border: none; padding: 1rem 2rem; border-radius: 30px; font-weight: 600; cursor: pointer; font-size: 1.1rem; transition: all 0.3s ease; display: flex; align-items: center; gap: 8px;">
                        <i class="fas fa-chart-line"></i>
                        Check Progress Status
                    </button>
                    <button onclick="location.href='/HTML/Home Page/index-eng.html'" style="background: var(--light-color); color: var(--primary-color); border: 2px solid var(--primary-color); padding: 1rem 2rem; border-radius: 30px; font-weight: 600; cursor: pointer; font-size: 1.1rem; transition: all 0.3s ease; display: flex; align-items: center; gap: 8px;">
